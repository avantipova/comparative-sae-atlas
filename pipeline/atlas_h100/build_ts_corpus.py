...
def main():
    ...
    A = ad.concat(parts, join="inner", merge="same", index_unique="-")   # merge="same" keeps identical var cols
    # var: keep Ensembl + symbols robustly (census var index is usually the Ensembl feature_id)
    if "feature_id" in A.var.columns:
        ens = A.var["feature_id"].astype(str).tolist()
    else:
        ens = [str(x) for x in A.var_names]
    A.var["ensembl_id"] = [e.split(".")[0] for e in ens]
