...
# controls.json is shared with controls2/3/4 and the recalibration scripts: merge into the existing file,
# never replace it (a plain dump once wiped random_null, survivor_nulls, stats_final and ten other blocks)
_prev = json.load(open(f"{C}/controls.json")) if os.path.exists(f"{C}/controls.json") else {}
_prev.update(out)
json.dump(_prev, open(f"{C}/controls.json", "w"), indent=1)
