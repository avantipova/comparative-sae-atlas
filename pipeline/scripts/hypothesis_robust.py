...
def same_family(a, b):
    """Two symbols that differ only in a short suffix after a shared stem of >= 3 characters:
    ACTB/ACTG2, S100A11/S100A4, ATP5ME/ATP5MK, HLA-A/HLA-B. The earlier rule -- one regex on each
    symbol -- matched none of those three examples from its own docstring, so variant R1 dropped
    only a fraction of the pairs it claimed to drop. This is a naming rule, not HGNC families; it
    errs on the side of dropping (CDC42/CDC37 count as one family), which is the safe direction
    for a control that asks whether trivial pairs carry the result."""
    a, b = a.upper(), b.upper()
    if a == b:
        return True
    n = 0
