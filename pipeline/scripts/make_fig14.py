...
Mat = np.full((len(M), len(M)), np.nan)   # the diagonal is self-similarity, not a measurement
for r in CK:
    p, q = r["pair"].split("-")
...
a = ax[0]
_cmap = mpl.cm.get_cmap("Blues").copy(); _cmap.set_bad("#F2F2F2")
# scale over the measured pairs only: with the diagonal pinned at 1 the real range (0.09-0.84) was compressed
im = a.imshow(Mat, cmap=_cmap, vmin=min(_real), vmax=max(_real))
for i in range(len(M)):            # 45 pairs fit; a heatmap without its numbers cannot be read off
    for j in range(len(M)):
        if i == j or np.isnan(Mat[i, j]):
            continue
        v = Mat[i, j]
        a.text(j, i, f"{v:.2f}".lstrip("0"), ha="center", va="center", fontsize=4.6,
               color="white" if v > (min(_real) + max(_real)) / 2 else "#222")
_D = {"AIDO": "AIDO.Cell", "C2S": "C2S-Scale", "Geneformer": "Geneformer-V2", "Tahoe": "Tahoe-x1"}
_lab = [_D.get(m, m) for m in M]
a.set_xticks(range(len(M))); a.set_xticklabels(_lab, rotation=90, fontsize=6.5)
a.set_yticks(range(len(M))); a.set_yticklabels(_lab, fontsize=6.5)
...
a.text(0.0, -0.52, f"all 45 pairs null-tested: real {min(_real):.2f}–{max(_real):.2f} vs cell-shuffle "
                   f"{min(_shuf):.3f}–{max(_shuf):.3f}\n(worst pair still {min(r['real_cka']/r['shuffled_cka'] for r in CK):.0f}× its own shuffle floor)",
...
b.set_xticks(x); b.set_xticklabels([_D.get(m, m) for m in mods], rotation=90, fontsize=7)
...
MCOL = {"AIDO": "#E69F00", "C2S": "#009E73", "Geneformer": "#56B4E9", "MaxToki": "#CC79A7",
        "UCE": "#0072B2", "scGPT": "#D55E00", "tGPT": "#117733", "scFoundation": "#882255",
        "GeneCompass": "#AA4499", "Tahoe": "#333333"}     # same palette as Fig S1
for m in atlas["models"]:
    ...
    c.plot(xs, ys, "-", color=MCOL.get(m, BLUE), lw=1.1, alpha=0.85, label=_D.get(m, m))
...
c.set_title("C  Tissue identity is linearly readable", loc="left", fontweight="bold", fontsize=9.5)
c.text(0.02, 0.45, "MLP−linear gap ≈ 0 at every depth", transform=c.transAxes, fontsize=7, color="#555")
c.legend(frameon=False, fontsize=5.6, loc="lower right", ncol=2, handlelength=1.0,
         columnspacing=0.8, labelspacing=0.25, borderaxespad=0.3)
...
# 1) models
box(1, 40, 15, 30, "10 single-cell\nfoundation models\n(10M – 3B)\nreads a cell as: rank ·\nmagnitude · protein\nprior: none · ESM ·\nknowledge · text", fc="#EEF3FA", fs=7.5)
# 2) corpus
...
box(11, 6, 61, 9, "Annotation-FREE (orthogonal, pass their own nulls), on the residual stream itself:\ncross-model CKA · linear readout · tissue×depth   —   and, on the dictionary, SAE > PCA",
    fc="#F3F0FA", ec="#b0a0d0", fs=7.3)
arrow(35.5, 46, 35.5, 15)
ax.text(36.5, 30, "residual stream", fontsize=6.8, color="#7A7A7A", rotation=90, va="center")
