...
# The fold is a ratio of counts: with 10 recovered edges it is far less determined than with 59.
# Whiskers are the 95 % Poisson interval on the recovered-edge count, divided by the same null.
_lo, _hi = [], []
for m in ms:
    d = per[m]; h = d["hits"] or 0; nm = d["null_mean"] or 0
    if not h or not nm:
        _lo.append(0); _hi.append(0); continue
    half = 1.96 * np.sqrt(h)
    _lo.append(max((d["fold"] or 0) - max(h - half, 0) / nm, 0))
    _hi.append((h + half) / nm - (d["fold"] or 0))
ax1.errorbar(fold, y, xerr=[_lo, _hi], fmt="none", ecolor="#5a6169", elinewidth=.9, capsize=2.2, zorder=3)
...
for yi, m in zip(y, ms):
    ...
    _off = _hi[list(ms).index(m)] + .3
    ax1.text(max(d["fold"] or 0, 0) + _off, yi, lab, va="center", fontsize=7,
...
ax1.set_xlim(0, (max(f + h for f, h in zip(fold, _hi))) * 1.22)
ax1.set_title("A   Which models predict", loc="left", fontsize=9, fontweight="bold")
ax1.text(.98, .17, "blue: p ≤ 0.05 · orange: too few pairs to test\nwhiskers: 95 % Poisson interval on the\nrecovered-edge count",
         transform=ax1.transAxes, ha="right", va="top", fontsize=6.2, color="#5a6169", linespacing=1.35)
